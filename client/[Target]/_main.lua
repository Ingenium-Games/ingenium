...
local function RaycastFromCamera(flag)
...
-- Toggle ig.target, instead of holding the hotkey
local toggleHotkey = 0
...
local function enableTargeting()
    ...
    while isActive do
        ...
        if not hit or distance > MaxTargetDistance then
            -- Try alternate flag
            ...
            if not hit or distance > MaxTargetDistance then
                -- Still nothing, throttle and skip the rest
                Wait(25)
                goto terminatewhileisActive -- skip the rest of this loop iteration
            end
        end
        ...
        if hit and distance < 7 then
            ...
            for _, v in pairs(options) do
                ...
                for i = 1, optionCount do
                    ...
                    if not hide and option.interact then
                        hide = not option.interact(entityHit, distance, endCoords, option.name, bone)
                    end
...
if toggleHotkey then
    RegisterCommand('ig.target', function()
        if isActive then
...
local function getResponse(option, server)
    ...
    if server and response.entity then
        response.net = NetworkGetNetworkIdFromEntity(response.entity)
    end
