...
function MySQL.Async.fetchAll(query, parameters, callback)
    ...
    Citizen.CreateThread(function()
        local results = c.sql.Query(query, params)
...
function MySQL.Async.fetchScalar(query, parameters, callback)
    ...
    Citizen.CreateThread(function()
        local result = c.sql.FetchScalar(query, params)
...
function MySQL.Async.execute(query, parameters, callback)
    ...
    Citizen.CreateThread(function()
        ...
        if string.find(upperQuery, "^%s*INSERT") then
            result = c.sql.Insert(query, params)
        else
            result = c.sql.Update(query, params)
        end
...
function MySQL.Async.insert(query, parameters, callback)
    ...
    Citizen.CreateThread(function()
        ...
        if type(query) == 'number' or (type(query) == 'string' and tonumber(query)) then
            ...
            result = c.sql.ExecutePrepared(query, params)
        else
            -- Regular insert
            result = c.sql.Insert(query, params)
        end
...
function MySQL.Async.store(query, callback)
    Citizen.CreateThread(function()
        local queryId = c.sql.PrepareQuery(query)
...
function MySQL.Async.transaction(queries, callback)
    Citizen.CreateThread(function()
        local result = c.sql.Transaction(queries)
...
function MySQL.Sync.fetchAll(query, parameters)
    local params = parameters or {}
    return c.sql.Query(query, params)
end
...
function MySQL.Sync.fetchScalar(query, parameters)
    local params = parameters or {}
    return c.sql.FetchScalar(query, params)
end
...
function MySQL.Sync.execute(query, parameters)
    ...
    if string.find(upperQuery, "^%s*INSERT") then
        return c.sql.Insert(query, params)
    else
        return c.sql.Update(query, params)
    end
end
...
if SHOW_DEPRECATION_WARNINGS then
    local function wrapWithWarning(funcName, originalFunc)
        return function(...)
            print(string.format("^3[SQL DEPRECATION] %s is deprecated - migrate to c.sql.* API^7", funcName))
